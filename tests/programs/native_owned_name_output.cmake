...
if(NOT YOGI_NATIVE_CC OR NOT YOGI_NATIVE_AR)
	message(FATAL_ERROR "native-owned name output program requires a C compiler and ar/llvm-ar")
endif()

set(NATIVE_SOURCE "${TEST_WORK_DIR}/name_output_native.c")
set(NATIVE_OBJECT "${TEST_WORK_DIR}/name_output_native.o")
set(NATIVE_LIBRARY "${TEST_WORK_DIR}/libname_output_native.a")
...
if(NOT native_compile_result EQUAL 0)
	message(FATAL_ERROR "native-owned name output fixture compile failed:\nstdout:\n${native_compile_stdout}\nstderr:\n${native_compile_stderr}")
endif()
...
if(NOT native_archive_result EQUAL 0)
	message(FATAL_ERROR "native-owned name output fixture archive failed:\nstdout:\n${native_archive_stdout}\nstderr:\n${native_archive_stderr}")
endif()

set(SOURCE "${TEST_WORK_DIR}/main.ts")
file(WRITE "${SOURCE}" [=[
extern names from "./libname_output_native.a" {
...
if(NOT compile_result EQUAL 0)
	message(FATAL_ERROR "native-owned name output program compile failed:\nstdout:\n${compile_stdout}\nstderr:\n${compile_stderr}")
endif()
...
foreach(path IN ITEMS "${EXECUTABLE}" "${IR}" "${OBJECT}")
	if(NOT EXISTS "${path}")
		message(FATAL_ERROR "expected native-owned name output artifact was not generated: ${path}")
	endif()
endforeach()
...
foreach(symbol IN ITEMS getName destroyName yogi_string_from_native_owned)
	if(NOT ir MATCHES "${symbol}")
		message(FATAL_ERROR "expected native-owned name output IR to contain ${symbol}")
	endif()
endforeach()

if(NOT ir MATCHES "native.string.output.slot")
	message(FATAL_ERROR "expected native-owned name output IR to allocate a native output pointer slot")
endif()
...
if(NOT run_result EQUAL 0)
	message(FATAL_ERROR "native-owned name output executable failed:\nstdout:\n${run_stdout}\nstderr:\n${run_stderr}")
endif()

set(expected_stdout "Output Nova\n1\n")
if(NOT run_stdout STREQUAL expected_stdout)
	message(FATAL_ERROR "native-owned name output printed unexpected output:\nexpected:\n${expected_stdout}\nactual:\n${run_stdout}\nstderr:\n${run_stderr}")
endif()
