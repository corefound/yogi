...
namespace yogi::runtime {
    ...
    namespace {
        ...
        struct AllocationRecord {
            ...
            std::size_t attributionIndex = static_cast<std::size_t>(-1);
        ...
        constexpr std::size_t invalidAttributionIndex = static_cast<std::size_t>(-1);
        ...
        std::size_t findAttributionIndex(const char* moduleName, const char* functionName, const char* typeName, const char* sourcePath, std::size_t line, std::size_t column) {
            for (std::size_t index = 0; index < attributionCount; ++index) {
                if (std::strcmp(attributions[index].moduleName, moduleName) == 0 && std::strcmp(attributions[index].functionName, functionName) == 0 && std::strcmp(attributions[index].typeName, typeName) == 0 &&
                    std::strcmp(attributions[index].sourcePath, sourcePath) == 0 && attributions[index].line == line && attributions[index].column == column) {
                    return index;
                }
            }

            return invalidAttributionIndex;
        }

        std::size_t ensureAttributionIndex(const char* moduleName, const char* functionName, const char* typeName, const char* sourcePath, std::size_t line, std::size_t column) {
            ...
            const auto existingIndex = findAttributionIndex(safeModule, safeFunction, safeType, safeSource, line, column);

            if (existingIndex != invalidAttributionIndex) {
                return existingIndex;
            }

            if (attributionCount == attributionCapacity) {
                const auto nextCapacity = attributionCapacity == 0 ? 32 : attributionCapacity * 2;
                auto* nextAttributions = static_cast<AttributionStats*>(std::realloc(attributions, sizeof(AttributionStats) * nextCapacity));
            ...
            const auto index = attributionCount++;
            auto* stats = &attributions[index];
            *stats = {};
        ...
        AttributionStats* attributionAt(std::size_t index) {
            return index < attributionCount ? &attributions[index] : nullptr;
        }
        ...
        AllocationRecord* ensureRecord(void* address) {
            ...
            if (recordCount == recordCapacity) {
                ...
                auto* nextRecords = static_cast<AllocationRecord*>(std::realloc(records, sizeof(AllocationRecord) * nextCapacity));
        ...
        bool attributionMatches(const AttributionStats& stats, const char* moduleName, const char* functionName) {
            return std::strcmp(stats.moduleName, safeModuleName(moduleName)) == 0 && std::strcmp(stats.functionName, safeFunctionName(functionName)) == 0;
        }

        bool locationMatches(const AttributionStats& stats, const char* sourcePath, std::size_t line, std::size_t column) {
            return std::strcmp(stats.sourcePath, safeSourcePath(sourcePath)) == 0 && stats.line == line && stats.column == column;
        }
    } // namespace
    ...
    void MemoryTelemetry::recordAllocation(void* address, std::size_t size, const char* typeName) {
        ...
        record->allocationId = ProgramObservability::nextAllocationId();
        record->generation = 1;
#endif
        const auto& frame = currentFrame();
        const auto& source = currentSourceFrame();
        record->attributionIndex = ensureAttributionIndex(frame.moduleName, frame.functionName, typeName, source.sourcePath, source.line, source.column);
        record->alive = true;

        currentLiveBytes += size;
        currentLiveAllocations++;
        allocatedBytes += size;
        recordAttributedAllocation(attributionAt(record->attributionIndex), size);
        refreshPeak();

#if YOGI_ENABLE_PROGRAM_OBSERVABILITY
        ProgramObservability::recordAllocation(record->allocationId, record->generation, address, size, record->typeName, frame.moduleName, frame.functionName, source.sourcePath, source.line, source.column);
#endif
    }

    void MemoryTelemetry::recordReallocation(void* oldAddress, void* newAddress, std::size_t newSize, const char* typeName) {
        if (!oldAddress) {
...
#if YOGI_ENABLE_PROGRAM_OBSERVABILITY
        const auto allocationId = oldRecord && oldRecord->allocationId != 0 ? oldRecord->allocationId : ProgramObservability::nextAllocationId();
        const auto generation = oldRecord ? oldRecord->generation + 1 : 1;
#endif
        ...
        if (oldRecord && oldRecord->alive) {
            ...
            freedBytes += oldSize;
            recordAttributedDeallocation(attributionAt(oldRecord->attributionIndex), oldSize);
        }
...
#if YOGI_ENABLE_PROGRAM_OBSERVABILITY
        newRecord->allocationId = allocationId;
        newRecord->generation = generation;
#endif
        const auto& frame = currentFrame();
        const auto& source = currentSourceFrame();
        newRecord->attributionIndex = ensureAttributionIndex(frame.moduleName, frame.functionName, typeName, source.sourcePath, source.line, source.column);
        newRecord->alive = true;

        currentLiveBytes += newSize;
        currentLiveAllocations++;
        allocatedBytes += newSize;
        recordAttributedAllocation(attributionAt(newRecord->attributionIndex), newSize);
        refreshPeak();

#if YOGI_ENABLE_PROGRAM_OBSERVABILITY
        ProgramObservability::recordReallocation(allocationId, generation, oldAddress, newAddress, oldSize, newSize, newRecord->typeName, frame.moduleName, frame.functionName, source.sourcePath, source.line, source.column);
#endif
    }
    ...
    void MemoryTelemetry::recordDeallocation(void* address) {
...
#if YOGI_ENABLE_PROGRAM_OBSERVABILITY
        const auto allocationId = record->allocationId;
        const auto generation = record->generation;
        const auto size = record->size;
        const auto* typeName = record->typeName;
        const auto& frame = currentFrame();
        const auto& source = currentSourceFrame();
#endif
        ...
        recordAttributedDeallocation(attributionAt(record->attributionIndex), record->size);
        record->alive = false;

#if YOGI_ENABLE_PROGRAM_OBSERVABILITY
        ProgramObservability::recordDeallocation(allocationId, generation, address, size, typeName, frame.moduleName, frame.functionName, source.sourcePath, source.line, source.column);
#endif
    }
    ...
    void MemoryTelemetry::pushContext(const char* moduleName, const char* functionName) {
        if (contextCount == contextCapacity) {
            ...
            auto* nextStack = static_cast<ContextFrame*>(std::realloc(contextStack, sizeof(ContextFrame) * nextCapacity));
        ...
        auto* moduleCopy = copyString(safeModuleName(moduleName));
        auto* functionCopy = copyString(safeFunctionName(functionName));
        std::uint64_t frameId = 0;

#if YOGI_ENABLE_PROGRAM_OBSERVABILITY
        const auto& source = currentSourceFrame();
        frameId = ProgramObservability::beginFrame(currentFrame().frameId, moduleCopy, functionCopy, source.sourcePath, source.line, source.column);
#endif

        contextStack[contextCount++] = {moduleCopy, functionCopy, frameId};
    }

    void MemoryTelemetry::popContext(const char* exitReason) {
        if (contextCount == 0) {
            return;
        }
...
#if YOGI_ENABLE_PROGRAM_OBSERVABILITY
        const auto& source = currentSourceFrame();
        ProgramObservability::endFrame(frame.frameId, exitReason, frame.moduleName, frame.functionName, source.sourcePath, source.line, source.column);
#endif
    ...
    std::uint64_t MemoryTelemetry::currentFrameId() {
    ...
    void MemoryTelemetry::pushSourceLocation(const char* sourcePath, std::size_t line, std::size_t column) {
        if (sourceCount == sourceCapacity) {
            ...
            auto* nextStack = static_cast<SourceFrame*>(std::realloc(sourceStack, sizeof(SourceFrame) * nextCapacity));
    ...
    void MemoryTelemetry::report() {
        std::fprintf(stderr, "yogi memory telemetry: live_bytes=%zu live_allocations=%zu total_allocated_bytes=%zu total_freed_bytes=%zu peak_bytes=%zu\n", currentLiveBytes, currentLiveAllocations, allocatedBytes, freedBytes, highestLiveBytes);
