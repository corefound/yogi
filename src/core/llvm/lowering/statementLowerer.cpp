...
namespace yogi::core::llvm::internal {
    ...
    namespace {
        ...
        void deactivateReturnedResourceSources(const Yogi::Sir::ValueRef* value, ModuleLoweringContext& context) {
            if (!value) {
                return;
            }
            ...
            if (const auto* identifier = value->identifier()) {
                ...
                if (context.nativeResourceDestroyFunction(name).has_value() || !context.nativeResourceFieldDestroyFunctions(name).empty() || !context.nativeResourceArrayElementFieldDestroyFunctions(name).empty()) {
            ...
            if (const auto* call = value->call()) {
                if (fbString(call->builtin_method()) == "move" && call->arguments() && call->arguments()->size() > 0) {
                    deactivateReturnedResourceSources(call->arguments()->Get(0), context);
                }
                return;
            }
            ...
            if (const auto* object = value->object()) {
                if (object->properties()) {
                    for (const auto* property : *object->properties()) {
                        deactivateReturnedResourceSources(property->value(), context);
                    }
                }
                return;
            }
            ...
            if (const auto* array = value->array()) {
                if (array->elements()) {
                    for (const auto* element : *array->elements()) {
                        deactivateReturnedResourceSources(element, context);
                    }
                }
            }
        }
        ...
        bool isStringType(const Yogi::Sir::TypeRef* type) {
            ...
            const auto kind = type->resolved() ? type->resolved()->kind() : type->kind();
        ...
        bool isArrayType(const Yogi::Sir::TypeRef* type) {
            ...
            return kind == Yogi::Sir::TypeKind_array_type || kind == Yogi::Sir::TypeKind_tuple_type;
        }
    ...
    StatementLowerer::StatementLowerer(ModuleLoweringContext& context, TypeLowerer& types, ValueLowerer& values, VariableLowerer& variables) : context(context), types(types), values(values), variables(variables) {}
    ...
    void StatementLowerer::lowerModuleInitializer() {
        ...
        auto* function = ::llvm::Function::Create(functionType, ::llvm::Function::ExternalLinkage, "_yogi_module_init_" + context.moduleName(), context.module.get());
    ...
    void StatementLowerer::lowerModuleCleanup() {
        auto* functionType = ::llvm::FunctionType::get(::llvm::Type::getVoidTy(context.llvmContext), false);
        auto* function = ::llvm::Function::Create(functionType, ::llvm::Function::ExternalLinkage, "_yogi_module_cleanup_" + context.moduleName(), context.module.get());
        auto* entry = ::llvm::BasicBlock::Create(context.llvmContext, "entry", function);
        context.builder.SetInsertPoint(entry);
        ...
        for (const auto* node : *context.sirModule->nodes()) {
            ...
            if (value->getType()->isPointerTy()) {
                ...
                for (auto* destroyed : destroyedAggregates) {
                    alreadyDestroyed = context.builder.CreateOr(alreadyDestroyed, context.builder.CreateICmpEQ(value, destroyed));
                }

                auto* shouldDestroy = context.builder.CreateAnd(context.builder.CreateNot(isNull), context.builder.CreateNot(alreadyDestroyed));
    ...
    void StatementLowerer::lowerEntryPoint(const std::vector<std::string>& moduleInitializers, const std::vector<std::string>& moduleCleanups) {
        ...
        for (const auto& initializerName : moduleInitializers) {
            ...
            if (!initializer) {
                initializer = ::llvm::Function::Create(initializerType, ::llvm::Function::ExternalLinkage, initializerName, context.module.get());
            }
        ...
        for (auto cleanupName = moduleCleanups.rbegin(); cleanupName != moduleCleanups.rend(); ++cleanupName) {
            ...
            if (!cleanup) {
                cleanup = ::llvm::Function::Create(initializerType, ::llvm::Function::ExternalLinkage, *cleanupName, context.module.get());
            }
    ...
    void StatementLowerer::lowerStatement(const Yogi::Sir::SirNode* node) {
        ...
        if (const auto* call = node->value_as_CallExpression()) {
            if (values.lowerDiscardedArrayElementCall(call)) {
                return;
            }
            ...
            } else if (isArrayType(call->type()) && !callReturnsBorrowedArray(call) && !callReturnsReceiverArray(call)) {
                const auto temporaryOwner = "$discarded.array." + std::to_string(context.localAggregateCleanups.size());
    ...
    void StatementLowerer::lowerReturn(const Yogi::Sir::ReturnStatement* statement) {
        ...
        if (returnType->isVoidTy()) {
            emitLocalCleanups(true, "return");
            context.popMemoryContext("return");
            context.builder.CreateRetVoid();
            return;
        }

        context.pushMemorySourceLocation(statement->position());
        auto* returnValue = values.cast(values.lower(statement->value(), returnType, context.currentReturnType), returnType, context.currentReturnType);
        const auto materializeFixedShapeView = returnsFixedShapeView(statement->value());
        if (!materializeFixedShapeView && values.returnRequiresOwnedCopy(statement->value(), context.currentReturnType)) {
            returnValue = values.cast(
                values.copyOwnedValue(returnValue, context.currentReturnType),
                returnType,
                context.currentReturnType,
                context.currentReturnType);
        }
        if (materializeFixedShapeView) {
            auto* opaquePointer = ::llvm::PointerType::get(context.llvmContext, 0);
            auto* cloneFunction = context.runtimeFunction("yogi_array_clone", opaquePointer, {opaquePointer});
            auto* ownedCopy = context.builder.CreateCall(cloneFunction, {returnValue}, "return.array.clone");
        ...
        const auto* ownedReturnType = resolvedType(context.currentReturnType);
        const auto ownedReturnKind = ownedReturnType
            ? ownedReturnType->kind()
            : Yogi::Sir::TypeKind_unknown;
        const auto returnTransfersOwnership =
            values.isStructType(context.currentReturnType) ||
            ownedReturnKind == Yogi::Sir::TypeKind_string_type ||
            ownedReturnKind == Yogi::Sir::TypeKind_array_type ||
            ownedReturnKind == Yogi::Sir::TypeKind_tuple_type ||
            ownedReturnKind == Yogi::Sir::TypeKind_type_literal ||
            ownedReturnKind == Yogi::Sir::TypeKind_any_type ||
            ownedReturnKind == Yogi::Sir::TypeKind_union_type ||
            ownedReturnKind == Yogi::Sir::TypeKind_pointer_type;

        if (returnTransfersOwnership) {
            ...
            if (const auto* identifier = statement->value() ? statement->value()->identifier() : nullptr) {
                context.deactivateAggregateOwner(identifier->symbol_id());
            }
        }
        deactivateReturnedResourceSources(statement->value(), context);
    ...
    void StatementLowerer::emitLocalCleanups(bool preserveRegistrations, const std::string& exitReason) {
        emitLocalCleanupsFrom(0, preserveRegistrations, exitReason);
    }

    void StatementLowerer::emitLocalCleanupsFrom(std::size_t firstCleanup, bool preserveRegistrations, const std::string& exitReason) {
        ...
        for (auto index = context.localAggregateCleanups.size(); index > firstCleanup; --index) {
            ...
            context.recordCleanupEmission(cleanup, exitReason);

            if (!cleanup.runtimeDestroyFunction.empty()) {
                auto* value = cleanup.value;
                auto* slot = cleanup.cleanupSlot ? ::llvm::cast<::llvm::AllocaInst>(cleanup.cleanupSlot) : nullptr;

                if (slot) {
                    value = context.builder.CreateLoad(slot->getAllocatedType(), cleanup.cleanupSlot);
                }

                auto* isNull = context.builder.CreateIsNull(value);
                auto* currentBB = context.builder.GetInsertBlock();
                auto* function = currentBB->getParent();
                auto* cleanupBB = ::llvm::BasicBlock::Create(context.llvmContext, "runtime.cleanup", function);
                auto* nullBB = ::llvm::BasicBlock::Create(context.llvmContext, "runtime.skip", function);
                auto* mergeBB = ::llvm::BasicBlock::Create(context.llvmContext, "runtime.done", function);
                context.builder.CreateCondBr(isNull, nullBB, cleanupBB);
                context.builder.SetInsertPoint(cleanupBB);
                context.emitRuntimeCleanupEvent(cleanup, "cleanup.execute", exitReason);

                auto* destroy = context.runtimeFunction(cleanup.runtimeDestroyFunction, ::llvm::Type::getVoidTy(context.llvmContext), {::llvm::PointerType::getUnqual(context.llvmContext)});
                context.builder.CreateCall(destroy, {value});

                if (slot) {
                    context.builder.CreateStore(::llvm::Constant::getNullValue(slot->getAllocatedType()), cleanup.cleanupSlot);
                }

                context.builder.CreateBr(mergeBB);
                context.builder.SetInsertPoint(nullBB);
                context.emitRuntimeCleanupEvent(cleanup, "cleanup.skip", exitReason);
                context.builder.CreateBr(mergeBB);
            ...
            if (cleanup.cleanupSlot) {
                ...
                auto* loaded = context.builder.CreateLoad(::llvm::PointerType::getUnqual(context.llvmContext), cleanup.cleanupSlot);
                auto* isNull = context.builder.CreateIsNull(loaded);
                auto* currentBB = context.builder.GetInsertBlock();
                auto* function = currentBB->getParent();
                auto* cleanupBB = ::llvm::BasicBlock::Create(context.llvmContext, "agg.cleanup", function);
                auto* nullBB = ::llvm::BasicBlock::Create(context.llvmContext, "agg.skip", function);
                auto* mergeBB = ::llvm::BasicBlock::Create(context.llvmContext, "agg.done", function);
                context.builder.CreateCondBr(isNull, nullBB, cleanupBB);
                context.builder.SetInsertPoint(cleanupBB);
                context.emitRuntimeCleanupEvent(cleanup, "cleanup.execute", exitReason);
                ...
                context.builder.CreateStore(::llvm::Constant::getNullValue(::llvm::cast<::llvm::AllocaInst>(cleanup.cleanupSlot)->getAllocatedType()), cleanup.cleanupSlot);
                context.builder.CreateBr(mergeBB);
                context.builder.SetInsertPoint(nullBB);
                context.emitRuntimeCleanupEvent(cleanup, "cleanup.skip", exitReason);
                context.builder.CreateBr(mergeBB);
    ...
    void StatementLowerer::lowerIf(const Yogi::Sir::IfStatement* statement) {
        ...
        auto* elseBlock = statement->else_block() ? ::llvm::BasicBlock::Create(context.llvmContext, "if.else", function) : nullptr;
    ...
    void StatementLowerer::lowerWhile(const Yogi::Sir::WhileStatement* statement) {
        ...
        if (context.localAggregateCleanups.size() > cleanupStart) {
            context.localAggregateCleanups.resize(cleanupStart);
        }
    ...
    void StatementLowerer::lowerFor(const Yogi::Sir::ForStatement* statement) {
        ...
        if (context.localAggregateCleanups.size() > bodyCleanupStart) {
            context.localAggregateCleanups.resize(bodyCleanupStart);
        }
    ...
    void StatementLowerer::lowerBreak(const Yogi::Sir::BreakStatement*) {
        ...
        emitLocalCleanupsFrom(frame.breakCleanupStart, false, "break");
    ...
    void StatementLowerer::lowerContinue(const Yogi::Sir::ContinueStatement*) {
        ...
        emitLocalCleanupsFrom(frame.continueCleanupStart, false, "continue");
    ...
    void StatementLowerer::lowerSwitch(const Yogi::Sir::SwitchStatement* statement) {
        ...
        for (::flatbuffers::uoffset_t i = 0; i < numClauses; ++i) {
            ...
            const auto bodyLabel = isCase ? "switch.case" + std::to_string(i) + ".body" : "switch.default.body";
        ...
        for (auto pos : casePositions) {
            checkBBs.push_back(::llvm::BasicBlock::Create(context.llvmContext, "switch.check" + std::to_string(pos), function));
        }
        ...
        for (std::size_t ci = 0; ci < checkBBs.size(); ++ci) {
            ...
            auto* caseValue = values.lower(caseClause->expression(), ::llvm::Type::getDoubleTy(context.llvmContext));
            auto* cmp = context.builder.CreateFCmpOEQ(discriminant, caseValue, "switch.cmp" + std::to_string(casePositions[ci]));
        ...
        for (::flatbuffers::uoffset_t i = 0; i < numClauses; ++i) {
            ...
            const auto* body = clauseInfos[i].isCase ? clause->value_as_CaseClause()->body() : clause->value_as_DefaultClause()->body();
