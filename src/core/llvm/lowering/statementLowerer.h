...
namespace yogi::core::llvm::internal {
    ...
    class StatementLowerer {
       public:
        StatementLowerer(ModuleLoweringContext& context, TypeLowerer& types, ValueLowerer& values, VariableLowerer& variables);

        void predeclareStructs();
        void lowerModuleInitializer();
        void lowerModuleCleanup();
        void lowerEntryPoint(const std::vector<std::string>& moduleInitializers, const std::vector<std::string>& moduleCleanups);
        ...
        void emitLocalCleanups(bool preserveRegistrations = false, const std::string& exitReason = "normal");
        void emitLocalCleanupsFrom(std::size_t firstCleanup, bool preserveRegistrations = false, const std::string& exitReason = "normal");
